...
def SecStructure(args):
    ...
    lstm = LSTM(output_dim=args.num_categories, input_shape=(None, example_len, num_channels),
                return_sequences=True, W_regularizer=None)(masking)
    ...
    dense = Dense(output_dim=example_len*args.num_categories,
                  activity_regularizer=None)(flat)
    ...
    nadam = Nadam(lr=args.learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-08,
                  schedule_decay=0.004)

    model.compile(optimizer=nadam, loss=vector_categorical_crossentropy,
                  metrics=[vector_categorical_accuracy])
...
# custom metric: categorical accuracy for a vector of predictions
