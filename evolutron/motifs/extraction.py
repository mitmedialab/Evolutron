...
from evolutron.tools import num2aa, hot2num, data_it
...
def motif_extraction(motif_fun, x_data, handle, filters=None, filter_size=None):
    foldername = 'motifs/' + str(handle).split('.')[0]
    ...
    for k, filt in enumerate(max_seq_scores):
        ...
        for i, seq in enumerate(filt):
            if seq[0] > seq_mean:
                j = int(seq[1])
                if j + filter_size - 1 <= x_data[i].shape[1]:
                    matches[k].append(num2aa(hot2num(x_data[i][:, j:j + filter_size])))
